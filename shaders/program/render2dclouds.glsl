...
#include "/include/shared/skyProjection.glsl"
	...
	void main() {
		...
		float depth = texelFetch(depthtex1, pixel, 0).x;
		...
		if (render_clouds) {
			...
			#endif
			viewVector = mat3(gbufferModelViewInverse) * GetViewDirection(positionScreen.xy, gbufferProjectionInverse);

			float lowerLimitDistance = RaySphereIntersection(viewPosition, viewVector, atmosphere_lowerLimitRadius).x;
			render_clouds = lowerLimitDistance <= 0.0 || eyeAltitude >= CLOUDS2D_ALTITUDE;
		}
