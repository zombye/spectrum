vec3 screenSpaceToViewSpace(vec3 position, mat4 projectionInverse) {
	...
	return (mat3(projectionInverse) * position + projectionInverse[3].xyz) / (position.z * projectionInverse[2].w + projectionInverse[3].w);
}
vec3 viewSpaceToScreenSpace(vec3 position, mat4 projection) {
	return ((mat3(projection) * position + projection[3].xyz) / position.z) * -0.5 + 0.5;
}
